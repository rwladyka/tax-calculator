...
export function TaxCalculatorPage() {
  ...
  const taxResults = useMemo(() => {
    if (!input?.annualIncome || !input?.taxYear || !data?.tax_brackets?.length) return null;

    return calculateTax(input?.annualIncome, data?.tax_brackets, input?.taxYear);
  }, [input, data]);

  return (
    <div className="mx-auto max-w-2xl px-4 py-10">
      <header className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">Income Tax Calculator</h1>
        <p className="mt-1 text-sm text-gray-500">Calculate income tax based on tax brackets.</p>
      </header>
